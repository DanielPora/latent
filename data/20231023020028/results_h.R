...
ggplot(filter(df, method != 'LCA 2'), aes(x = obs_per_trial, y = NMIc_mean, color = method)) +
  geom_boxplot()+
  facet_wrap(~ scenario, ncol=3)+
  ylab("NMIc")+
...
ggplot(filter(df, method != 'LCA 2'), aes(x = obs_per_trial, y = RIc_mean, color = method)) +
  geom_boxplot()+
  facet_wrap(~ scenario, ncol=3)+
  ylab("RIc")+
  xlab("Observations per trial type")+
...
ggplot(filter(df, method != 'LCA 2'), aes(x = obs_per_trial, y = NMI_mean, color = method)) +
  geom_boxplot()+
  facet_wrap(~ scenario, ncol=3)+
  ylab("NMI")+
  xlab("Observations per trial type")+
...
ggplot(filter(df, method != 'LCA 2'), aes(x = obs_per_trial, y = RI_mean, color = method)) +
  geom_boxplot()+
  facet_wrap(~ scenario, ncol=3)+
  ylab("RI")+
  xlab("Observations per trial type")+
